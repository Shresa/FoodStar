      ...
      <div className="lg:hidden bg-white shadow-lg p-3 fixed bottom-0 z-10 w-full flex items-center justify-between md:justify-evenly text-gray-500 border">
        {allTypes.map((items) => (
            <Link to={`/${items.id}`}>
                ...
                className={
                    type === items.id
                    ? "flex flex-col relative items-center text-xl text-foodstar-400 "
                ...
                <div
                    className={
                    type === items.id &&
                    "absolute -top-3 w-full h-2 border-t-2 border-foodstar-400"
                    }
                />
                {items.icon}
        ...
        {allTypes.map((items) => (
            <Link to={`/${items.id}`}>
                ...
                className={classnames(
                    ...
                    {
                    "border-b-2 border-foodstar-400": type === items.id,
                    }
                )}
                >
                <div
